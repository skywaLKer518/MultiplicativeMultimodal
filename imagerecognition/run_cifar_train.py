...
flags.DEFINE_float("margin", 0.0, "margin for boosted mul")
FLAGS = flags.FLAGS
...
def evaluate(sess, model, data_iter, violations=None):
  """Runs evaluation."""
  num_correct = 0.0
  count = 0
  if violations:
  ...
  for batch in data_iter:
    res = model.infer_step(sess, batch["img"], violations=violations, label=batch["label"])
    if violations:
      y, n_vio = res
      n_violations += n_vio
    else:
    ...
    with tf.Session() as sess:
      ...
      for niter in tqdm(range(niter_start, config.max_train_iter), desc=exp_id):
        ...
        if (niter + 1) % config.valid_iter == 0 or niter == 0:
          ...
          test_iter.reset()
          acc, n_violations = evaluate(sess, mvalid, test_iter, violations=True)
          exp_logger.log_valid_acc(niter, acc)
	  if config.multimodal in [11, 12, 13, 1,2]:
            exp_logger.log_value(niter, 'n_violations', n_violations, 'number of violations', 'violations')
